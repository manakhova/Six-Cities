...
const NewCommentForm = (props) => {
  ...
  const [validity, setValidity] = useState({comment: false, rating: false});
  const [userForm, setUserReview] = useState({comment: ``, rating: null});
  ...
  const handleSubmit = (evt) => {
    ...
    // setDisabedStatus(true);
    setUserReview({comment: ``, rating: null});
    setValidity({comment: false, rating: false});
    ...
    <form className="reviews__form form" action="#" method="post" onSubmit={handleSubmit}>
      ...
      <div className="reviews__rating-form form__rating">
        <input className="form__rating-input visually-hidden" name="rating" value="5" id="5-stars" type="radio"
          ...
          disabled={!isDisabled ? false : true} required/>
      ...
      <textarea minLength={`${MIN_COMMENT_LENGTH}`} maxLength={`${MAX_COMMENT_LENGTH}`}
        ...
        disabled={!isDisabled ? false : true} required></textarea>
      <div className="reviews__button-wrapper">
        <p className="reviews__help">
      To submit review please make sure to set <span className="reviews__star">rating</span> and describe your stay with at least <b className="reviews__text-amount">50 characters</b>.
        </p>
        <button className="reviews__submit form__submit button" type="submit" disabled={validity.rating && validity.comment && !isDisabled ? false : true}>Submit</button>
      </div>
    </form>
  );
};
